...
class TestNewFeatures(unittest.TestCase):
    ...
    def test_identity_service_duplicates(self):
        ...
        clip1 = Clipping(content="Foo bar", book_title="B", author="A", date_time=datetime.now(), location="10")
        clip2 = Clipping(content="Foo bar", book_title="B", author="A", date_time=datetime.now(), location="10")
        
        self.assertTrue(IdentityService.is_duplicate(clip1, clip2), "Identical clips should be duplicates")

        # Fuzzy test (location matches, content slightly different)
        clip3 = Clipping(content="Foo bar.", book_title="B", author="A", date_time=datetime.now(), location="10") 
        self.assertTrue(IdentityService.is_duplicate(clip1, clip3), "Similar content at same location should be duplicate")
